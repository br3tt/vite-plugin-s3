...
var S3_PATH_SEP = "/";
var PATH_SEP = import_node_path.default.sep;
...
function getDirectoryFilesRecursive(dir, ignores = []) {
  return new Promise((resolve2, reject) => {
    (0, import_recursive_readdir.default)(dir, ignores, (error, files) => {
      if (error)
        reject(error);
      else
...
function createContext(options2) {
  const basePath = options2.basePath ? addTrailingS3Sep(options2.basePath) : "";
...
function logResult(files, vite) {
  ...
  if (logLevel === "info") {
    console.info([
      ...
      ...files.map((p) => `${dim((0, import_node_path2.relative)(root, p.path))}`)
    ].join("\n"));
  }
}
...
var Uploader = class {
  constructor(options2, vite) {
    this.options = options2;
    this.vite = vite;
    this.client = new import_client_s3.S3(this.options.clientConfig);
    this.directory = this.options.directory ? this.options.directory : (0, import_path.resolve)(this.vite.root, this.vite.build.outDir);
  }
  uploadFile(fileName, file) {
    let Key = this.options.basePath + fileName;
    const params = (0, import_mapValues.default)(this.options.uploadOptions, (optionConfig) => {
      return typeof optionConfig === "function" ? optionConfig(fileName, file) : optionConfig;
    });
    if (Key[0] === "/")
      Key = Key.substr(1);
    if (params.ContentType === void 0)
      params.ContentType = (0, import_mime_types.lookup)(fileName) || "application/octet-stream";
    const Body = import_node_fs.default.createReadStream(file);
    return this.client.putObject(__spreadValues(__spreadValues({
      Key,
      Body
    }, DEFAULT_UPLOAD_OPTIONS), params));
  }
...
// src/index.ts
function ViteS3(enabled2, userOptions) {
  const options2 = createContext(userOptions);
  let vite;
  return {
    name: "vite-plugin-s3",
    enforce: "post",
    apply(config, { command }) {
      return command === "build" && enabled2;
    },
    ...
    closeBundle: {
      async handler() {
        if (!vite.build.ssr && enabled2) {
          const uploader = new Uploader(options2, vite);
